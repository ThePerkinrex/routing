...
async fn main() {
    ...
    chassis_b.add_network_layer_process(chassis::NetworkLayerId::Ipv4, IpV4Process {
    ...
    for IpV4Process
{
    ...
    async fn on_up_message(
        ...
        for (id, sender) in down_sender {
            ...
            sender.send_async(ProcessMessage::Message(NetworkLayerId::Ipv4, (BROADCAST, Ipv4Packet::new(ipv4::addr::BROADCAST, self.ip, vec![0x69, 0x69]).to_vec()))).await.unwrap();
        }
    }
}
