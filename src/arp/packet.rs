...
impl ArpPacket {
    pub fn new_request(htype: u16, ptype: EtherType, sha: Vec<u8>, spa: Vec<u8>, tpa: Vec<u8>) -> Self {
        let hlen = get_hardware_len(htype).unwrap_or_else(|| unimplemented!("Unknown htype {htype:?}"));
		let plen = get_protocol_len(ptype).unwrap_or_else(|| unimplemented!("Unknown ptype {ptype:?}"));
	...
	pub fn new_reply(htype: u16, ptype: EtherType, sha: Vec<u8>, spa: Vec<u8>, tha: Vec<u8>, tpa: Vec<u8>) -> Self {
        let hlen = get_hardware_len(htype).unwrap_or_else(|| unimplemented!("Unknown htype {htype:?}"));
		let plen = get_protocol_len(ptype).unwrap_or_else(|| unimplemented!("Unknown ptype {ptype:?}"));
		if sha.len() != hlen as usize {
			panic!("SHA size ({}) doesnt match expected ({hlen})", sha.len());
    ...
    pub fn from_vec(data: &[u8]) -> Option<Self> {
		...
		let tha = data[(8+hlen as usize+plen as usize)..(8+(hlen as usize)*2+plen as usize)].to_vec();
		let tpa = data[(8+(hlen as usize)*2+plen as usize)..(8+(hlen as usize)*2+(plen as usize)*2)].to_vec();
        Some(Self { htype, ptype, hlen, plen, operation, sender_harware_address: sha, sender_protocol_address: spa, target_harware_address: tha, target_protocol_address: tpa })
    }
